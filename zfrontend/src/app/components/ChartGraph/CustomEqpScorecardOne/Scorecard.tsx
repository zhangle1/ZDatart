...
class Scorecard extends ReactChart {
  ...
  constructor(props?) {
    super(ScorecardAdapter, {
      id: props?.id || 'custom-eqp-react-scorecard-one',
      name: props?.name || 'viz.palette.graph.names.customerEqpScoreChartOne',
  ...
  getOptions(
    ...
    const chainConfigs = dataConfigs
      .filter(c => c.key === 'chain')
      .flatMap(config => config.rows || []);
    ...
    const chainData: ChartsEventData[] = [
      {
        name: getColumnRenderName(chainConfigs[0]),
        value: toFormattedValue(
          chartDataSet?.[0]?.getCell?.(chainConfigs[0]),
          chainConfigs[0]?.format,
        ),
        ...getExtraSeriesRowData(chartDataSet?.[0]),
      },
    ];
